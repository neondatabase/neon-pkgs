...
Describe a CLI run in telemetry only with the credential the global auth middleware selected for
it.

Three defects, all from the same cause — the internal user id and the key to ask the API about
were chosen by separate expressions, neither of which asked whether `ensureAuth` had selected a
credential at all:
...
- Nothing identified the run, which is the normal state in CI and on a fresh machine, and also
  ...
  `accountId: ""` under `authMethod: "oauth"`, naming a method for a run with no account to name.
  Both fields are now omitted unless a credential named the account.
