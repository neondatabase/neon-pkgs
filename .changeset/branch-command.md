...
Add `checkout <branch>` for selecting an existing branch without creating anything, and make `branch [name]` always create a new branch from the policy (`dev` becomes `dev-*`, omitted name becomes `*`, with the wildcard filled by git branch + mini id). `pull` now prints selected branch state as JSON for inspection/copy-paste, while `init` writes a starter `neon.ts` policy file.
