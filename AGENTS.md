...
-   **`neon init` is a thin orchestrator** in `src/commands/init.ts`. It re-execs existing commands: empty directory (only `.git`) → `bootstrap . --no-link` then `skills` then `link` (if `.neon` has no projectId) then `mcp`; otherwise `skills` then `link` (if needed) then `mcp`. `-y` forwards to those commands. Bootstrap is always `--no-link` so the `link` child owns the project pin. `src/init/plan.ts` is the pure planner; `src/init/bootstrap.ts` is the template scaffolding core that `commands/bootstrap.ts` uses. Init itself skips `ensureAuth`; the child commands authenticate.
