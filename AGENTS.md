...
-   **`neon init` is a thin orchestrator** in `src/commands/init.ts`. It re-execs existing commands: empty directory (only `.git`) → `bootstrap .` then `skills update` then `mcp`; otherwise `skills` then `link` (if `.neon` has no projectId) then `mcp`. `-y` forwards to those commands. `src/init/plan.ts` is the pure planner; `src/init/bootstrap.ts` is the template scaffolding core that `commands/bootstrap.ts` uses. Init itself skips `ensureAuth`; the child commands authenticate.
