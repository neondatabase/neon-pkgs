...
- Every command exposes flags for every interactive question so it can run with no TTY (`--agent`, `--project-id`, `--skill`, `--global`, `--oauth`, `--project`, `--template`, `--no-agent-setup`, `--project-setup`, …). Coding-agent targeting is `--agent <name>` on `skills`, `plugins`, `mcp`, `init`, and `bootstrap`; init Recommended detection is global config, project folders, and the host CLI. `init` may install the plugin and skills/MCP in one run; `bootstrap` still refuses that mix. `link` has no `--agent`.
...
-   **`neon init` is one flow** in `src/commands/init.ts` / `src/init/run.ts`. A TTY asks Recommended vs Custom. `-y` is Recommended: plugin (or mixed plugin + skills/MCP) for detected agents (global config, project folders, host CLI), or Cursor+Codex skills in this directory when none are detected; link when authenticated; default neon.ts via in-process `config init` (`silent`, `requireInstall`). `-y` with `--skill`, MCP flags, `--no-agent-setup`, or `--project-setup` is Custom: those flags, Recommended defaults for the rest. `--skill` is skills (not plugin); MCP flags are skills and MCP. Unauthenticated `-y` skips link and prints signup / `neon auth` / `neon link` / `neon claim create`. Interactive Custom keeps plugin vs skills+MCP vs skip, then link or claimable (72h) if unauthenticated, then neon.ts services; package manager is inferred and asked only when none is detected. Mixed is allowed on init, not on bootstrap. `--template` still nested-bootstraps (`printBanner: false`, `linkNoConfig`, `skipDoneSummary`); empty-dir `-y` no longer scaffolds (use `--template` or `neon bootstrap`). `--agent` skips agent selection and implies Custom when `-y` is omitted. `--no-link` skips linking. Init records `cli_init_start` / `cli_init_end`. `src/init/copy.ts` owns prompt strings. `src/init/chrome.ts` owns the banner and completion summary (stdout, not `log.info`). `src/init/plan.ts` is the planner; `src/init/choices.ts` is template/config flag resolution; `src/init/wizard.ts` is the pickers; `src/init/tooling.ts` runs the children; `src/init/link.ts` authenticates and runs link in process; `src/init/bootstrap.ts` is the template scaffolding core. Init itself skips global `ensureAuth`. After a new default neon.ts on a pinned branch, init runs child `env pull` unless link already pulled env or the file declares extra services.
