...
-   **`neon init` is one flow** in `src/commands/init.ts`. Empty directory (only `.git`) asks how to set up the directory: a starter template, or skip scaffolding. That skip is init-only; `neon bootstrap` always scaffolds. Template choice calls `bootstrap` in-process (`printBanner: false`, `linkNoConfig`, `skipDoneSummary`). `--skip-template` and skip-template use the existing-app path. Existing app → plugin or skills+MCP (never both), including target-agent selection and setup, then the shared in-process link flow if `.neon` has no projectId (or if `--project-id` / `--org-id` / `--branch` were typed — those flags are not filled from `.neon`), then one neon.ts decision. Interactive bootstrap also finishes agent setup before asking whether to link. Saying no skips the services picker and does not write the file. `--agent` / `-a` on init and bootstrap is forwarded to plugins, or to skills and mcp, not both, and skips agent selection. `-y` MCP is `mcp -y` (global), not `--project`. Dependency install is last except when the template has `neon.ts` and link needs deps first. `src/init/chrome.ts` owns the NEON banner and the completion summary (stdout, not `log.info`). `src/init/plan.ts` is the planner; `src/init/choices.ts` is template/config flag resolution; `src/init/wizard.ts` is the init pickers; `src/init/tooling.ts` runs the children; `src/init/link.ts` authenticates and runs link in process; `src/init/bootstrap.ts` is the template scaffolding core that `commands/bootstrap.ts` uses. Init itself skips global `ensureAuth`; the link flow authenticates lazily after consent. After a newly written neon.ts on a pinned branch, init runs child `env pull`. Interactive template picks pass the catalog entry into nested bootstrap so scaffolding uses the fetched source, not the bundled fallback for that id.
