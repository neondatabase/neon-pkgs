...
- `-y` / `--yes` / `--default` is the non-interactive path. If it cannot decide, the error names the flag (and values) to pass. `neon link` with no project: pass `--project-id <id>`. `neon skills -y` with no agent: run from a supported agent, or drop `-y` to pick.
- Every command exposes flags for every interactive question so it can run with no TTY (`--project-id`, `--skill`, `--global`, `--oauth`, `--project`, `--template`, …). Coding-agent targeting is detection on `-y` (project folders, else the host CLI; `--global -y` uses installed apps, else the host), or the picker without `-y`.
