...
const runSetupSql = async (): Promise<void> => {
	...
	if (r.status !== 0) {
		throw new Error(
			`setup SQL failed: exit=${r.status} stderr=${r.stderr}`,
		);
	}
};
...
describe.skipIf(!SHOULD_RUN)("tap/010_tab_completion", () => {
	...
	it("complete my<tab> to mytab (multiple choices, line 143)", async () => {
		...
		await checkCompletion("select * from my\t", /select \* from mya?tab/);
	});
	...
	it("complete prefix of a GUC name — set plpg<tab> → plpgsql. (line 380)", async () => {
		await checkCompletion("set plpg\t", /plpga?sql\./);
	});
