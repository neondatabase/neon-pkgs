...
import { defaultRules, type NormalizeRule, normalize } from "./normalize.js";
...
describe("normalize: per-rule coverage", () => {
	...
	it("iso-timestamp masks PG and ISO-8601 timestamp shapes", () => {
		...
		expect(normalize("2026-05-25 14:23:10 +00:00 foo")).toBe(
			"TIMESTAMP foo",
		);
	});

	it("notice-pid masks bracketed pids in NOTICE prefixes", () => {
		expect(normalize("NOTICE:  [12345]: oops")).toBe(
			"NOTICE:  [PID]: oops",
		);
	});
	...
	it("regress-abs-builddir masks the psql-conformance-regress prefix", () => {
		...
		expect(
			normalize(
				"/tmp/psql-conformance-regress-abc123/results/psql-output1",
			),
		).toBe("ABS_BUILDDIR/results/psql-output1");
	});
