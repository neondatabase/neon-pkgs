...
export const defaultRules: readonly NormalizeRule[] = [
	...
	// ERROR line so it cannot match anything in expected output.
	{
		...
		pattern:
			/ERROR: {2}REINDEX CONCURRENTLY cannot be executed within a pipeline/g,
		replacement:
	...
	// REINDEX above.
	{
		...
		pattern: /ERROR: {2}VACUUM cannot be executed within a pipeline/g,
	...
	// occurs in this specific test scenario.
	{
		...
		pattern:
			/WARNING: {2}SET LOCAL can only be used in transaction blocks\n( statement_timeout \n-------------------\n 2h\n)/g,
		replacement: "$1",
		pgMajorAtMost: 17,
	},
	...
	// error (when LOCK is the first command in the pipeline).
	{
		...
		pattern:
			/ 1\n\(1 row\)\n\nERROR: {2}LOCK TABLE can only be used in transaction blocks\n/g,
		replacement: " 1\n(1 row)\n\n ?column? \n----------\n 2\n(1 row)\n\n",
		pgMajorAtMost: 17,
	},
];
...
export function normalize(
	...
	for (const rule of rules) {
		...
		if (rule.pgMajorAtLeast !== undefined) {
			if (pgMajor === undefined || pgMajor < rule.pgMajorAtLeast)
				continue;
		}
