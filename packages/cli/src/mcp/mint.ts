...
const assertUsable = async (
	...
	scope: { orgId: string | null; projectId?: string },
	fallbackName: string,
	...
	const problem =
		typeof data.key !== "string" || data.key.trim() === ""
			...
			: data.project_id !== wanted
				? `Neon returned a key scoped to ${data.project_id ?? "nothing"} rather than ${wanted ?? "the whole account"}.`
				: !Number.isSafeInteger(data.id)
					? "Neon returned no key id."
					: null;
	if (problem) {
		const withdrawn = await withdraw(client, scope.orgId, data.id);
		throw new Error(
			`${problem} ${
				withdrawn
...
export async function mintMcpApiKey(options: {
	...
	const { apiClient, projectId } = options;

	if (!projectId) {
		try {
			const { data } = await apiClient.createApiKey({ key_name: name });
			return assertUsable(apiClient, data, { orgId: null }, name);
		} catch (err) {
			if (
				isNeonApiError(err) &&
	...
	const orgId = await orgIdForProject(apiClient, projectId);
	const { data } = await apiClient.createOrgApiKey(orgId, {
		key_name: name,
		project_id: projectId,
	});
	return assertUsable(apiClient, data, { orgId, projectId }, name);
}
...
export async function withdrawMintedKey(
	...
	return withdraw(apiClient, key.orgId, key.id);
}
