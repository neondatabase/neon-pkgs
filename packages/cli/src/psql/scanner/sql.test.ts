...
const corpus: CorpusCase[] = [
	...
	},
	{
		...
		input: "CREATE FUNCTION f() RETURNS int LANGUAGE plpgsql AS $body$\nBEGIN\n  RETURN 1;\nEND;\n$body$;",
		expectedSplits: [
	...
	// block, embedded `;` does not terminate the surrounding CREATE.
	{
		...
		input: "CREATE FUNCTION psql_df_sql (x integer)\n  RETURNS integer\n  BEGIN ATOMIC SELECT x + 1; END;",
		expectedSplits: [
	...
	},
	{
		...
		input: "CREATE OR REPLACE FUNCTION f(x int) RETURNS int BEGIN ATOMIC SELECT x; SELECT x + 1; END;",
		expectedSplits: [
	...
	},
	{
		...
		input: "CREATE FUNCTION f(x int) RETURNS int BEGIN ATOMIC SELECT CASE x WHEN 1 THEN 1; ELSE 0; END CASE; END;",
		expectedSplits: [
	...
	// bodies (`BEGIN ATOMIC ... END`).
	{
		...
		input: "CREATE FUNCTION add(integer, integer) RETURNS integer AS 'select $1 + $2;' LANGUAGE SQL;",
		expectedSplits: [
	...
	},
	{
		...
		input: "CREATE FUNCTION f() RETURNS int AS 'BEGIN; x := 1; RETURN x; END;' LANGUAGE plpgsql;\nSELECT f();",
		expectedSplits: [
