...
describe("defaultSettings — special-variable hooks", () => {
	...
	describe("ON_ERROR_ROLLBACK", () => {
		test("rejects unknown value with upstream multi-line wording", () => {
			...
			expect(r).toEqual({
				ok: false,
				reason: "hook-veto",
				error: 'unrecognized value "foo" for "ON_ERROR_ROLLBACK"\nAvailable values are: on, off, interactive.',
			});
		});
	...
	describe("ECHO", () => {
		test("rejects unknown value", () => {
			...
			expect(
				typeof r === "object" && !r.ok && r.reason === "hook-veto",
			).toBe(true);
		});
	...
	describe("ECHO_HIDDEN", () => {
		test("rejects unknown value with three-element list", () => {
			...
			expect(r).toEqual({
				ok: false,
				reason: "hook-veto",
				error: 'unrecognized value "maybe" for "ECHO_HIDDEN"\nAvailable values are: on, off, noexec.',
			});
		});
	...
	describe("SHOW_ALL_RESULTS", () => {
		test("rejects non-boolean", () => {
			...
			expect(r).toEqual({
				ok: false,
				reason: "hook-veto",
				error: 'unrecognized value "foo" for "SHOW_ALL_RESULTS": Boolean expected',
			});
		});
	});
	...
	describe("WATCH_INTERVAL", () => {
		...
		test('negative values emit "must be greater than 0.00" suffix', () => {
			...
			expect(r).toEqual({
				ok: false,
				reason: "hook-veto",
				error: 'invalid value "-1" for variable "WATCH_INTERVAL": must be greater than 0.00',
			});
		});
		...
		test('over-max values emit "must be less than 1000000.00" suffix', () => {
			...
			expect(r).toEqual({
				ok: false,
				reason: "hook-veto",
				error: 'invalid value "2000000" for variable "WATCH_INTERVAL": must be less than 1000000.00',
			});
		});
