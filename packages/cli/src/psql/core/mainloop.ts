...
import type { IfState, REPLContext } from "../types/repl.js";
...
import { type PromptContext, renderPromptByName } from "./prompt.js";
...
const makeEditorLineReader = async (
	...
	return {
		...
		pushHistory: (line: string): void => {
			...
			void appendHistory(histPath, trimmed, histControl).catch(
				() => undefined,
			);
		},
		interject: (text: string): void => {
	...
	const processChunk = async (chunk: string): Promise<void> => {
		...
		while (working.length > 0) {
			...
			if (result.kind === "backslash") {
				...
				// user's mental model.
				const slashOnlyLine =
					result.sql.length === 0 && queryBuf.length === 0;
				queryBuf += result.sql;
				working = working.slice(result.consumed);
