...
export class PgConnection implements Connection {
	...
	public static async connect(opts: ConnectOptions): Promise<PgConnection> {
		...
		if (opts.loadBalanceHosts === "random") {
			shuffleInPlace(
				candidates,
				PgConnection._loadBalanceRng ?? Math.random,
			);
		}
		...
		try {
			...
			if (opts.ssl !== "verify-full") {
				...
				tlsConnectionOptions.checkServerIdentity = (
					...
					tls.checkServerIdentity(opts.host, cert);
			}
	...
	public getTlsInfo(): {
		...
		try {
			...
			const alpnRaw = (s as unknown as { alpnProtocol?: string | false })
				.alpnProtocol;
			const alpn =
				typeof alpnRaw === "string" && alpnRaw.length > 0
					? alpnRaw
					: null;
			return {
				...
				alpn,
				library: "OpenSSL",
				keyBits: sslKeyBitsFromCipher(
					cipher.standardName ?? cipher.name,
				),
			};
		} catch {
			return null;
	...
	public async query(sql: string, params?: unknown[]): Promise<ResultSet> {
		...
		if (params === undefined) {
			...
			if (sets.length === 0) {
				throw new Error(
					"PgConnection.query: server returned no result sets",
				);
			}
	...
	public async prepare(
		...
		return {
			...
			async bind(
				...
				conn.socket.write(
					Bind("", name, paramFormats ?? [], encoded, [0]),
				);
	...
	public async cancel(): Promise<void> {
		...
		try {
			...
			if (this.opts.ssl !== "verify-full") {
				...
				cancelTlsOpts.checkServerIdentity = (
					_h,
					cert,
				): Error | undefined => tls.checkServerIdentity(host, cert);
			}
	...
	public async setClientEncoding(name: string): Promise<void> {
		...
		await this.execSimple(
			`SET client_encoding TO ${this.escapeLiteral(name)}`,
		);
	}
		...
		switch (msg.type) {
			...
			case "AuthenticationSASL": {
				...
				try {
					...
					const { mechanism, clientFirstMessage } =
						this.scram.start();
					this.passwordUsed = true;
					this.socket.write(
						SASLInitialResponse(mechanism, clientFirstMessage),
					);
				} catch (err) {
	...
	private handleAwaitReady(msg: BackendMessage): void {
		switch (msg.type) {
			...
			default:
				this.failStartup(
					new Error(
						`Unexpected message ${msg.type} during connection startup`,
					),
				);
				return;
		}
	...
	private makeCopyInStream(): CopyInStream {
		...
		if (!driver) {
			throw new Error(
				"PgConnection: makeCopyInStream called without driver",
			);
		}
		return {
			write: (chunk: Buffer | string): Promise<void> => {
				...
				if (driver.closed) {
					return Promise.reject(
						new Error("CopyInStream already closed"),
					);
				}
				const data =
					typeof chunk === "string"
	...
	private makeCopyOutStream(): CopyOutStream {
		...
		if (!driver) {
			throw new Error(
				"PgConnection: makeCopyOutStream called without driver",
			);
		}
				...
				return {
					async next(): Promise<IteratorResult<Buffer>> {
						for (;;) {
							...
							if (isClosed()) {
								throw new Error(
									"Connection closed mid-COPY-OUT",
								);
							}
							await new Promise<void>((resolve) => {
	...
	private handleQueryMessage(msg: BackendMessage): void {
		...
		switch (msg.type) {
			...
			case "DataRow": {
				...
				q.current.rows.push(
					decodeDataRow(msg.values, q.current.fields),
				);
				return;
			}
			case "CommandComplete": {
			...
			case "ReadyForQuery": {
				...
				if (q.error) {
					...
					(
						err as Error & { partialResults?: ResultSet[] }
					).partialResults = q.finished;
					q.reject(err);
				} else {
			...
			case "CopyInResponse": {
				...
				try {
					this.socket.write(
						CopyFail("COPY FROM STDIN not driven by client"),
					);
				} catch {
			...
			case "CopyData": {
				...
				q.error = {
					severity: "ERROR",
					message:
						"Unexpected backend message during query: CopyData",
				};
				return;
			}
			case "CopyDone": {
				...
				q.error = {
					severity: "ERROR",
					message:
						"Unexpected backend message during query: CopyDone",
				};
				return;
			}
		...
		switch (this.state) {
			...
			case "idle":
				...
				this.socketError = new Error(
					`Unexpected ${msg.type} in idle state`,
				);
				try {
					this.socket.destroy();
				} catch {
	...
	private enqueueOp<T>(opSkeleton: ExtOp): Promise<T> {
		if (!this.extDriver) {
			return Promise.reject(
				new Error("enqueueOp: not in extended state"),
			);
		}
		const driver = this.extDriver;
	...
	private handleExtendedMessage(msg: BackendMessage): void {
		...
		if (!head) {
			this.protocolFail(
				new Error(
					`Unexpected backend message ${msg.type} in in-extended`,
				),
			);
			return;
		}
		...
		switch (msg.type) {
			case "ParseComplete":
				if (head.kind !== "parse") {
					this.protocolFail(
						new Error(
							"ParseComplete arrived but head op is " + head.kind,
						),
					);
					return;
				}
			...
			case "BindComplete":
				if (head.kind !== "bind") {
					this.protocolFail(
						new Error(
							"BindComplete arrived but head op is " + head.kind,
						),
					);
					return;
				}
			...
			case "CloseComplete":
				if (head.kind !== "close") {
					this.protocolFail(
						new Error(
							"CloseComplete arrived but head op is " + head.kind,
						),
					);
					return;
				}
			...
			case "ParameterDescription":
				if (head.kind !== "describeS") {
					this.protocolFail(
						new Error(
							"ParameterDescription arrived but head op is " +
								head.kind,
						),
					);
					return;
			...
			case "RowDescription":
				...
				this.protocolFail(
					new Error(
						"Unexpected RowDescription at head op " + head.kind,
					),
				);
				return;
			case "NoData":
			...
			case "DataRow": {
				if (head.kind !== "execute") {
					this.protocolFail(
						new Error("DataRow at head op " + head.kind),
					);
					return;
				}
...
function applyTlsProtocolVersionRange(
	...
	opts: Pick<
		ConnectOptions,
		"sslMinProtocolVersion" | "sslMaxProtocolVersion"
	>,
): void {
