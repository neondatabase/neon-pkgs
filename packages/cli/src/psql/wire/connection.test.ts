			...
			socket.on("data", (chunk: Buffer) => {
				...
				for (;;) {
					if (!startupSeen) {
						...
						if (code === 196608) {
							const params = parseStartup(
								accum.subarray(0, declared),
							);
							accum = accum.subarray(declared);
							startupSeen = true;
...
describe("PgConnection", () => {
	...
	test("runs execSimple(SELECT 1) end-to-end", async () => {
		server = await startFakeServer((msg, client) => {
			if (msg.type === "Startup") {
				...
				client.send(
					rowDescription([{ name: "?column?", oid: 23, size: 4 }]),
				);
	...
	test("SASL/SCRAM-SHA-256 happy path", async () => {
		...
		server = await startFakeServer((msg, client) => {
			...
			if (msg.type === "SASLInitial") {
				...
				client.send(
					authenticationSASLContinue(Buffer.from(serverFirst)),
				);
				return;
			}
	...
	test("startCopyIn → fail() surfaces server ErrorResponse", async () => {
		...
		await expect(
			copyIn.fail("client requested fail"),
		).rejects.toMatchObject({
			severity: "ERROR",
			code: "57014",
		});
	...
	test("execSimple sends CopyFail when no COPY-FROM-STDIN block is queued", async () => {
		...
		await expect(
			conn.execSimple("COPY t FROM STDIN"),
		).rejects.toMatchObject({
	...
	test("query(sql, params) drives Parse/Bind/Describe/Execute/Sync", async () => {
		...
		server = await startFakeServer((msg, client) => {
			...
			if (msg.type === "Describe") {
				client.send(
					rowDescription([{ name: "greeting", oid: 25, size: -1 }]),
				);
				return;
			}
			if (msg.type === "Execute") {
	...
	test("prepare() returns a usable PreparedStatement", async () => {
		...
		server = await startFakeServer((msg, client) => {
			...
			if (msg.type === "Describe") {
				...
				client.send(
					rowDescription([{ name: "col", oid: 23, size: 4 }]),
				);
				return;
			}
			if (msg.type === "Bind") {
	...
	test("closePreparedStatement on an unknown name succeeds quietly", async () => {
		...
		await expect(
			conn.closePreparedStatement("nope"),
	...
	test("CopyInResponse during execSimple while pipeline active aborts the connection", async () => {
		...
		(
			conn as unknown as { _extPipelineActive: boolean }
		)._extPipelineActive = true;
	...
	test("startCopyIn during pipeline rejects synchronously and closes the connection", async () => {
		...
		await expect(
			conn.startCopyIn("COPY t FROM STDIN"),
		).rejects.toMatchObject({
			message: "COPY in a pipeline is not supported, aborting connection",
		});
		expect(conn.isClosed()).toBe(true);
	...
	const startRoleServer = async (input: {
		...
		return startFakeServer((msg, client) => {
			...
			if (msg.type === "Query") {
				client.send(
					rowDescription([
						{ name: "pg_is_in_recovery", oid: 16, size: 1 },
					]),
				);
...
describe("tlsServername (sslsni)", () => {
	test("sends servername = host by default (sslsni unset)", () => {
		expect(tlsServername({ host: "db.example.com" })).toBe(
			"db.example.com",
		);
	});
...
describe("keepAliveArgs (keepalives / keepalives_idle)", () => {
	...
	test("disabled but with an idle delay still reports the delay", () => {
		expect(
			keepAliveArgs({ keepalives: false, keepalivesIdle: 10 }),
		).toEqual({
			enable: false,
