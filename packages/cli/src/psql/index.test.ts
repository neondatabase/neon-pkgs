...
describe("parseConnectionUri — target_session_attrs / load_balance_hosts", () => {
	...
	it("rejects unrecognised load_balance_hosts value", () => {
		expect(() =>
			parseConnectionUri(
				"postgresql://h/db?load_balance_hosts=roundrobin",
			),
...
describe("parseConnectionUri — gssencmode (GSSAPI unsupported)", () => {
	...
	it("accepts gssencmode=disable (no-op)", () => {
		const got = parseConnectionUri(
			"postgresql://host/db?gssencmode=disable",
		);
	...
	it("accepts gssencmode=prefer (no-op, falls back to non-GSS)", () => {
		const got = parseConnectionUri(
			"postgresql://host/db?gssencmode=prefer",
		);
		expect(got.host).toBe("host");
	});
...
describe("parseConnectionUri — libpq strict validation", () => {
	...
	it("rejects multiple bare query keys", () => {
		expect(() =>
			parseConnectionUri("postgresql://host?value1&value2"),
		).toThrow(/missing "="/);
	});
	...
	it('rejects extra "=" in query value', () => {
		expect(() =>
			parseConnectionUri("postgresql://host?key=key=value"),
		).toThrow(/extra "="/);
	});
	...
	it("rejects invalid percent-encoded token in query value (%XX)", () => {
		expect(() =>
			parseConnectionUri("postgres://host?dbname=%XXfoo"),
		).toThrow(/invalid percent-encoded token/);
	});
...
describe("parseConnectionUri — unix-domain socket conformance", () => {
	it("postgresql://?host=/path/to/socket/dir", () => {
		const got = parseConnectionUri(
			"postgresql://?host=/path/to/socket/dir",
		);
...
describe("parseConnectionUri — sslcertmode", () => {
	it("threads sslcertmode=disable into ConnectOptions", () => {
		const got = parseConnectionUri(
			"postgresql://u@h/db?sslcertmode=disable",
		);
	...
	it("threads sslcertmode=require into ConnectOptions", () => {
		const got = parseConnectionUri(
			"postgresql://u@h/db?sslcertmode=require",
		);
...
describe("envConnectionDefaults — PGSSLCERTMODE", () => {
	it("maps PGSSLCERTMODE=disable onto sslcertmode", () => {
		expect(
			envConnectionDefaults({ PGSSLCERTMODE: "disable" }),
		).toMatchObject({
			sslcertmode: "disable",
		});
	});
...
describe("envConnectionDefaults — PGSSLNEGOTIATION", () => {
	it("maps PGSSLNEGOTIATION=direct onto sslnegotiation", () => {
		expect(
			envConnectionDefaults({ PGSSLNEGOTIATION: "direct" }),
		).toMatchObject({ sslnegotiation: "direct" });
	});

	it("rejects a bogus PGSSLNEGOTIATION value", () => {
		expect(() =>
			envConnectionDefaults({ PGSSLNEGOTIATION: "bogus" }),
		).toThrow('invalid sslnegotiation value: "bogus"');
	});
});
...
describe("defaultClientCertDefaults", () => {
	const makeHome = (opts: {
		...
		const home = fs.mkdtempSync(
			path.join(os.tmpdir(), "neonctl-psql-home-"),
		);
...
describe("sslsni / keepalives / requirepeer parsing", () => {
	...
	it("parses sslsni=1 / keepalives=1 as true", () => {
		const got = parseConnectionUri(
			"postgresql://u@h/db?sslsni=1&keepalives=1",
		);
	...
	it("accepts on/off and yes/no spellings (libpq bool grammar)", () => {
		expect(
			parseConnectionUri("postgresql://u@h/db?sslsni=off").sslsni,
		).toBe(false);
		expect(
	...
	it("omits the fields when the query parameters are empty / absent", () => {
		const got = parseConnectionUri(
			"postgresql://u@h/db?sslsni=&keepalives=",
		);
	...
	it("parses them from a conninfo keyword/value string", () => {
		expect(
			parseConninfo(
				"sslsni=0 keepalives=0 keepalives_idle=7 requirepeer=pg",
			),
		).toMatchObject({
			sslsni: false,
			keepalives: false,
...
describe("parseConnectionUri — ssl protocol version range", () => {
	...
	it("rejects a malformed protocol-version value", () => {
		expect(() =>
			parseConnectionUri(
				"postgresql://u@h/db?ssl_min_protocol_version=TLSv9",
			),
...
describe("parseConnectionUri — internal whitespace in a query value", () => {
	...
	it("rejects port = 12345 12 with libpq integer-value wording", () => {
		expect(() =>
			...
			'invalid integer value "12345 12" for connection option "port"',
		);
	});
});
...
describe("parseConninfo", () => {
	...
	it("normalises replication=on to true", () => {
		expect(parseConninfo("replication=on")).toEqual({
			replication: "true",
		});
	});
...
describe("looksLikeConnectionString", () => {
	...
	it("detects key=value conninfo shape", () => {
		...
		expect(looksLikeConnectionString("host=h port=5432 dbname=d")).toBe(
			true,
		);
	});
