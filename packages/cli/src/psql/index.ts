...
export const runPsql = async (
	...
	try {
		...
		if (preActions.length > 0) {
			return await runPreActions(
				ctx,
				preActions,
				parsed.singleTransaction,
			);
		}
...
const splitAuthorityTuples = (rest: string, uri: string): string[] => {
	...
	while (i < rest.length) {
		...
		if (ch === "[") {
			...
			if (closeIdx < 0) {
				throw new Error(
					`missing matching "]" in IPv6 host address: ${uri}`,
				);
			}
			i = closeIdx + 1;
			continue;
...
export const parseConninfo = (input: string): Partial<ConnectOptions> => {
	...
	assertTlsProtocolRange(
		out.sslMinProtocolVersion,
		out.sslMaxProtocolVersion,
	);
...
const applyConninfoPair = (
	...
	switch (key) {
		...
		case "service": {
			...
			(out as Partial<ConnectOptions> & { _service?: string })._service =
				value;
			return;
		}
		default:
...
const assertTlsProtocolRange = (
	...
	if (
		TLS_PROTOCOL_VERSIONS.indexOf(min) > TLS_PROTOCOL_VERSIONS.indexOf(max)
	) {
		throw new Error(
...
const normalizeTargetSessionAttrs = (
	...
	switch (value) {
		...
		default:
			throw new Error(
				`invalid value for "target_session_attrs": "${raw}"`,
			);
	}
};
...
export const parseConnectionUriPartial = (
	...
	const cb = normalizeChannelBinding(
		raw.query.get("channel_binding") ?? null,
	);
	if (cb !== undefined) out.channelBinding = cb;
