...
const renderHorizontal = (
	...
	if (wrapped) {
		...
		if (maxColumns > 0) {
			...
			const widthAverage: number[] = new Array(colCount).fill(
				0,
			) as number[];
	...
	for (let r = 0; r < cellGrid.length; r++) {
		...
		// line.
		const colLineKinds: ("start" | "wrap" | "nl")[][] = row.map(
			(cell, i) => {
...
const renderRecordHeader = (
	...
	if (leftCore.length <= leftSegLen) {
		...
		// corner.
		const leftPadded =
			leftCore + hrule.repeat(leftSegLen - leftCore.length);
		return (
			outerLeft +
			leftPadded +
			outerMid +
