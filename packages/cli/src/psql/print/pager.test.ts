...
describe("isPagerNeeded", () => {
	test("returns false when pager is 'off'", () => {
		expect(isPagerNeeded(baseOpts({ pager: "off", lines: 1000 }))).toBe(
			false,
		);
	});
	...
	test("returns true when pager is 'always' with a non-empty command", () => {
		expect(
			isPagerNeeded(baseOpts({ pager: "always", pagerCmd: "cat" })),
		).toBe(true);
	});
	...
	test("whitespace-only PAGER falls through to the default", () => {
		...
		expect(
			isPagerNeeded(
				baseOpts({ pager: "always", env: { PAGER: "   \t  " } }),
			),
		).toBe(true);
	});
	...
	test("on Windows with no pager set, returns false even when 'always'", () => {
		if (!isWindows) return;
		expect(isPagerNeeded(baseOpts({ pager: "always", env: {} }))).toBe(
			false,
		);
	});
});
