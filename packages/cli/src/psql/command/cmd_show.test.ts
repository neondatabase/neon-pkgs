...
describe("cmdShowFunction (\\sf)", () => {
	test("no connection → error", async () => {
		...
		expect(stderrChunks.join("")).toMatch(
			/\\sf: no connection to the server/,
		);
	});
			...
			[
				{
					match: () => true,
					err: Object.assign(
						new Error('function "nope" does not exist'),
						{
							severity: "ERROR",
...
describe("cmdShowView (\\sv)", () => {
	test("no connection → error", async () => {
		...
		expect(r.status).toBe("error");
		expect(stderrChunks.join("")).toMatch(
			/\\sv: no connection to the server/,
		);
	});
			...
			[
				{
					match: (sql) =>
						sql.includes(
							"'foo'::pg_catalog.regproc::pg_catalog.oid",
						),
					result: mkResultSet([[10]]),
				},
				{
