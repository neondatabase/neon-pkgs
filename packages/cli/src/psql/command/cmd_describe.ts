...
const applyTwoPatterns = (
	...
	for (const result of results) {
		...
		const renumbered = conds.map((c) =>
			c.replace(
				/\$(\d+)/g,
				(_, n: string) => `$${Number(n) + slotOffset}`,
			),
		);
...
const makeDescribeCmd = (baseName: string): BackslashCmdSpec => ({
	...
	run: async (ctx) => {
		...
		const forceExpanded =
			ctx.cmdName.slice(baseName.length).includes("x") &&
			pattern === null;
...
const cmdDescribeOperators = (cmdName: string): BackslashCmdSpec => ({
	...
	run: async (ctx) => {
		...
		return runFunctionOrOperatorQuery(
			ctx,
			pattern,
			argPatternResults,
			query,
		...
		try {
			...
			if (rs.rows.length === 0 && !ctx.settings.quiet) {
				if (rolePat !== null && dbPat !== null) {
					...
					writeErr(
						`Did not find any settings for role "${rolePat}".\n`,
					);
				}
				return { status: "ok" };
			}
