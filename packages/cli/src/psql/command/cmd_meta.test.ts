...
describe("\\prompt", () => {
	...
	test('a leading "-" selects the no-echo (password) read path', async () => {
		...
		const r = await run(cmdPrompt, ctx);
		expect(r.status).toBe("ok");
		expect(readLineMock).toHaveBeenCalledWith("Password: ", {
			echo: false,
		});
