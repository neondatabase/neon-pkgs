...
import type { PipelineSession } from "../wire/pipeline.js";
			...
			const realFromSlice = ((): unknown => {
				for (const r of sliceForError) {
					...
					const isAborted =
						...
						reason !== null &&
						(reason as { pipelineAborted?: boolean })
				...
				if (r.status === "fulfilled") {
					...
					if (!isSyncOrPlaceholder && !isCommandOnly) {
						if (rs.fields.length === 0 && rs.rows.length > 0) {
							...
							const tuplesOnly =
								ctx.settings.popt.topt.tuplesOnly;
							if (!tuplesOnly) {
...
export const cmdGetResults: BackslashCmdSpec = {
	...
	async run(ctx: BackslashContext): Promise<BackslashResult> {
		...
		const resultAvailable = readCounter(
			ctx.settings,
		...
		const slots: readonly Promise<ResultSet>[] = Array.isArray(
			session.cmdSlots,
		)
			? session.cmdSlots
			: [];
