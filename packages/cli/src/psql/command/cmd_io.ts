...
const isSpeciallyTreatedVar = (settings: PsqlSettings, name: string): boolean =>
	settings.vars.hasSubstituteHook(name) ||
...
export const cmdWrite: BackslashCmdSpec = {
	...
	async run(ctx: BackslashContext): Promise<BackslashResult> {
		...
		try {
			...
			if (entry.isPipe) {
				const msg = formatChildWaitResult(
					result.exitCode,
					result.signal,
				);
				if (msg !== null) {
		...
		if (bindState?.byName) {
			...
			if (!ps) {
				...
				execError = Object.assign(
					new Error(
						`prepared statement "${bindState.name}" does not exist`,
					),
		...
		for (const rs of tupled) {
			for (const row of rs.rows) {
				for (const cell of row) {
					...
					try {
						const nested =
							await ctx.settings.db.execSimple(statement);
			...
			if (eqIdx > 0) {
				...
				if (key === "i") {
					...
					if (
						...
						return errResult(
							ctx,
							`incorrect interval value "${value}"`,
						);
					}
					interval = parsed;
					intervalSet = true;
				...
				if (key === "c") {
					...
					if (parsed === null || parsed === 0) {
						return errResult(
							ctx,
							`incorrect iteration count "${value}"`,
						);
					}
					iterMax = parsed;
					iterSet = true;
				...
				if (key === "m" || key === "min_rows") {
					...
					if (parsed === null) {
						return errResult(
							ctx,
							`incorrect minimum row count "${value}"`,
						);
					}
					minRows = parsed;
					minRowsSet = true;
