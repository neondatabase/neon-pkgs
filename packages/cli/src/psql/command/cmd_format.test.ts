...
const makeMockConnection = (): MockConnection => {
	...
	const conn: MockConnection = {
		...
		setClientEncoding(name: string): Promise<void> {
			...
			return conn.failWith
				? Promise.reject(conn.failWith)
				: Promise.resolve();
		},
...
describe("\\pset misc", () => {
	...
	test("linestyle preserves old-ascii (printer owns the glyph swap)", async () => {
		...
		await run(
			cmdPset,
			makeMockCtx("pset", "linestyle old-ascii", settings),
		);
	...
	test("csv_fieldsep diagnoses NUL/empty/multi-char vs forbidden singles", async () => {
		...
		const r2 = await run(
			...
			makeMockCtx(
				"pset",
				"csv_fieldsep ab",
				defaultSettings(createVarStore()),
			),
		);
	...
	test("extra-arg drain peels backtick delimiters but does not exec", async () => {
		...
		await run(
			cmdPset,
			makeMockCtx("pset", "fieldsep | `whoami`", settings),
		);
	...
	test("footer toggles defaultFooter and rejects non-boolean", async () => {
		...
		const r = await run(
			cmdPset,
			makeMockCtx("pset", "footer maybe", settings),
		);
		expect(r.status).toBe("error");
	});
	...
	test("xheader_width accepts full/column/page and positive int", async () => {
		...
		await run(
			cmdPset,
			makeMockCtx("pset", "xheader_width column", settings),
		);
...
describe("\\pset upstream wording", () => {
	...
	test('pager_min_lines pluralizes "line" vs "lines"', async () => {
		...
		expect(stdout()).toContain(
			"Pager won't be used for less than 0 lines.\n",
		);
		stdoutChunks.length = 0;
		await run(cmdPset, makeMockCtx("pset", "pager_min_lines 5", settings));
		expect(stdout()).toContain(
			"Pager won't be used for less than 5 lines.\n",
		);
	});
