...
import { type KeyEvent, Vt100Decoder } from "./vt100.js";
...
describe("Vt100Decoder", () => {
	...
	describe("escape sequences", () => {
		...
		it("arrow keys via CSI", () => {
			...
			expect(consume(bytes(0x1b, 0x5b, 0x43))).toEqual([
				{ key: "right" },
			]);
		...
		it("bracketed paste markers", () => {
			expect(consume(ascii("\x1b[200~"))).toEqual([
				{ key: "paste-start" },
			]);
	...
	describe("streaming", () => {
		...
		it("decodes multi-byte UTF-8 (two-byte)", () => {
			// 'é' is C3 A9 in UTF-8.
			expect(consume(bytes(0xc3, 0xa9))).toEqual([
				{ key: "char", char: "é" },
			]);
		});
