...
import type { CheckoutEvent, GitContext } from "@neon/config";

import { log } from "../log.js";
import { neonSafeBranchName } from "./branch_name.js";

/**
 * Env var the installed `post-checkout` hook sets before invoking `neon git sync`, so the
 * CLI can tell a hook-triggered run from a manual `neon checkout` / `deploy`. Read by
 * {@link buildCheckoutEvent} to construct the hook contexts' `event` field (see
 * `CheckoutEvent` in `@neon/config`) — not carried on {@link GitContext} itself.
 */
export const GIT_HOOK_ENV_FLAG = "NEON_GIT_HOOK";

/** Sentinel marking the `post-checkout` hook as neon-managed (safe to update/remove). */
const HOOK_SENTINEL = "# neon:git-sync (managed — edit `neon git` instead)";

const POST_CHECKOUT_HOOK = `#!/usr/bin/env sh
${HOOK_SENTINEL}
# Syncs the Neon branch to the checked-out git branch. Runs only on branch switches
# ($3 == 1), never blocks the checkout, and no-ops when neon isn't on PATH.
[ "$3" = "1" ] || exit 0
command -v neon >/dev/null 2>&1 || exit 0
${GIT_HOOK_ENV_FLAG}=1 neon git sync --quiet || true
`;
...
export const readGitContext = (cwd: string): GitContext => {
	...
	return {
		...
		...(branch
			? { branch, neonSafeBranchName: neonSafeBranchName(branch) }
			: {}),
		...(sha ? { sha } : {}),
		...(shortSha ? { shortSha } : {}),
 ...
 * `neon checkout` invocation, carrying whatever name/id the user passed (`undefined` for
...
/** Whether a `post-checkout` hook exists and is neon-managed (carries our sentinel). */
...
/**
 * Install (or refresh) the neon-managed `post-checkout` hook. Refuses to clobber a
