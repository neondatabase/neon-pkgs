...
const apiClient = {
	listProjectBranches: () =>
		Promise.resolve({ data: { branches: BRANCHES } }),
} as never;
...
describe("resolveBranchRef", () => {
	...
	it("resolves a branch by id and looks up its friendly name", async () => {
		expect(
			await resolveBranchRef(
				props({ branch: "br-feature-branch-000002" }),
			),
		).toEqual({
			branchId: "br-feature-branch-000002",
			branchName: "feature",
	...
	it("trusts a br- id the listing does not return (no friendlier name)", async () => {
		expect(
			await resolveBranchRef(
				props({ branch: "br-unknown-branch-999999" }),
			),
		).toEqual({
	...
	it("throws a helpful error when a branch name does not resolve", async () => {
		await expect(
			resolveBranchRef(props({ branch: "nope" })),
		).rejects.toThrow(
