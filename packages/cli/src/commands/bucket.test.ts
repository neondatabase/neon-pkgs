...
describe("bucket", () => {
	...
	test("object list", async ({ testCliCommand }) => {
		await testCliCommand(
			["bucket", "object", "list", "my-bucket", ...SCOPE],
			{
				mockDir: "single_org",
			},
		);
	...
	test("object ls alias", async ({ testCliCommand }) => {
		await testCliCommand(
			["bucket", "object", "ls", "my-bucket", ...SCOPE],
			{
				mockDir: "single_org",
			},
		);
	...
	test("object list --recursive with --delimiter is rejected client-side", async ({
		...
		await testCliCommand(
			...
			],
			{
				mockDir: "single_org",
				code: 1,
				stderr: "ERROR: --recursive and --delimiter cannot be used together. Use --recursive for a flat listing, or --delimiter to collapse on a separator.",
			},
		);
	});
	...
	test("object get downloads to the given file", async ({
		testCliCommand,
	}) => {
	...
	test("object get without a key is rejected client-side", async ({
		...
		await testCliCommand(
			["bucket", "object", "get", "my-bucket", ...SCOPE],
			{
				mockDir: "single_org",
				code: 1,
	...
	test("object put surfaces a clean status error when presign fails without a message body", async ({
		...
		await testCliCommand(
			...
			],
			{
				mockDir: "single_org",
				code: 1,
				stderr: 'ERROR: Failed to presign upload for "forbidden-nobody.txt" in bucket "my-bucket" on branch br-main-branch-123456 (HTTP 403): Request failed with status code 403',
			},
		);
	});
	...
	test("object delete --recursive rejects a prefix without a trailing slash", async ({
		...
		await testCliCommand(
			...
			],
			{
				mockDir: "single_org",
				code: 1,
				stderr: 'ERROR: Recursive delete requires a prefix ending in "/" (got "logs").',
			},
		);
	});
	...
	test("object delete --recursive rejects an empty prefix", async ({
		...
		await testCliCommand(
			...
			],
			{
				mockDir: "single_org",
				code: 1,
				stderr: 'ERROR: Recursive delete requires a non-empty prefix ending in "/".',
			},
		);
	});
});
