...
describe("vpc-endpoints", () => {
	...
	test("list org VPC endpoints implicit org", async ({ testCliCommand }) => {
		await testCliCommand(
			["vpc", "endpoint", "list", "--region-id", "test"],
			{
				mockDir: "single_org",
			},
		);
	...
	test("list project VPC endpoint restrictions", async ({
		testCliCommand,
	}) => {
		await testCliCommand(
			[
	...
	test("list project VPC endpoint restrictions with implicit project", async ({
		...
		await testCliCommand(
			["vpc", "project", "list", "--region-id", "test"],
			{
				mockDir: "single_org",
			},
		);
	...
	test("set project VPC endpoint restrictions", async ({
		testCliCommand,
	}) => {
		await testCliCommand(
