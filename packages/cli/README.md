...
`neon triggers` (alias `neon trigger`) manages Function triggers on a branch. Beta. Types are `schedule` (a five-field UTC cron expression) and `storage_object_created` (an object-storage bucket, with an optional object-key prefix).
