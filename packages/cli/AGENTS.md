...
Coding-agent targeting is `--agent <name>` (repeatable) on `skills`, `plugins`, and `mcp`; detection on `-y` (project folders, else the host CLI; `--global -y` uses installed apps, else the host); or omit `-y` in a terminal to pick. `init`, `link`, and `bootstrap` have no `--agent`.
