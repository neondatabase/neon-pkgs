...
Coding-agent targeting is `--agent <name>` (repeatable) on `skills`, `plugins`, `mcp`, `init`, and `bootstrap`; detection on `-y` (project folders, else the host CLI; `--global -y` uses installed apps, else the host); or omit `-y` in a terminal to pick. `init` and `bootstrap` pass `--agent` to plugins, or to skills and mcp, not both, and skip agent selection. Interactive init/bootstrap finish agent selection and setup before asking whether to link, then call link's implementation in process. `--no-link` skips that step without prompting on both commands. `link` has no `--agent`. `init -y` / `bootstrap --default` run `mcp -y` (global). Empty-dir `init` template skip is `--skip-template`. neon.ts on `init` is `--config` / `--no-config` (no default; omitted asks in a terminal). Those flags apply to skip-template and existing apps; scaffolding keeps the template's neon.ts. Init suppresses link's own neon.ts offer.
