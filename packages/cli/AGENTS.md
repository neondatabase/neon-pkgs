...
Coding-agent targeting is `--agent <name>` (repeatable) on `skills`, `plugins`, `mcp`, `init`, and `bootstrap`; detection on `-y` (project folders, else the host CLI; `--global -y` uses installed apps, else the host); or omit `-y` in a terminal to pick. `init` and `bootstrap` pass `--agent` to plugins, or to skills and mcp, not both, and skip agent selection. `link` has no `--agent`. `init -y` / `bootstrap --default` run `mcp -y` (global). Empty-dir `init` template skip is `--skip-template`. neon.ts on `init` is `--config` / `--no-config` (no default; omitted asks in a terminal). `link --no-config` skips that command's own neon.ts offer.
