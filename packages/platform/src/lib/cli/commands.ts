 ...
 * for connection strings, and writes them to `.env.local` (or the supplied filename) in
 * `KEY=value` format. Overwrites any existing file unconditionally — the file is meant
 * to be gitignored and machine-managed.
 */
...
function formatEnvFile(env: Parameters<typeof neonEnvToProcessEnv>[0]): string {
	const pairs = neonEnvToProcessEnv(env);
	const lines = [
		"# Generated by `neon-ts env pull`. Do not edit by hand — re-run to refresh.",
		"# The file is safe to commit-ignore (`.env.local` is in most framework defaults).",
		"",
	];
