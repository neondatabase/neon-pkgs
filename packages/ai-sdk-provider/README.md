...
- Gateway errors are re-emitted in the dialect each route's model parses, so `error.message` carries the reason rather than a bare `Bad Request`. The gateway returns its own OpenAI-shaped envelope, a flat `{ error_code, message }` from Databricks, and a `{ error_code, message }` wrapping an upstream error as a JSON string; only some of those parse on any given route.
