...
/**
 * Function Triggers (branch-scoped). `Trigger` is a discriminated union on
 * `type`; narrow on `t.type` to read a variant's fields. `create`/`update`
 * inputs must carry the `type` discriminator.
 */
export class Triggers<DThrow extends boolean> {
	...
	/**
	 * Update a trigger. `PATCH` is partial but must carry the `type`
	 * discriminator plus at least one field to change. Send `{ enabled: false }`
	 * to pause a trigger without deleting it.
	 *
