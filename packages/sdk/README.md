...
optional `response?: Response` and `request?: Request`, with the typed `NeonErrorUnion` on
the error channel. A `kind` check narrows it the same way as the ergonomic client.
`response` is `undefined` only when no HTTP response was received (network error, timeout,
abort). Pass `throwOnError: true` to get the bare resource and throw instead — and the
